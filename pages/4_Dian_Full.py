...
from graph.chains.retrieval import query_dianfull
...
st.set_page_config(
    page_title="Consultas sobre Dian Full",
    page_icon="📚",
    layout="wide"
)

# Título de la página
st.title("Consultas sobre Conceptos Completos de la Dian")

# Descripción de la página
st.markdown("""
Esta sección le permite realizar consultas sobre todos los conceptos de la Dian, incluyendo temas de IVA, Renta, Timbre y otros impuestos.
La base de conocimiento incluye conceptos de la Dian desde enero de 2017 hasta diciembre de 2024.
""")
...
try:
    ...
    index_name = "dianfull"  # Nombre específico del índice para Dian Full
    ...
    if not pinecone_api_key:
        ...
        if index_name not in existing_indexes:
            ...
            # Inicializar estado de sesión para Dian Full
            if "dianfull_messages" not in st.session_state:
                st.session_state.dianfull_messages = []
            ...
            for message in st.session_state.dianfull_messages:
            ...
            query = st.chat_input("Escribe tu consulta sobre cualquier concepto de la Dian...")
            
            # Procesar la consulta
            if query:
                # Agregar la consulta del usuario a los mensajes
                st.session_state.dianfull_messages.append({
                    "role": "user", 
                    "content": query
                })
                ...
                with st.chat_message("assistant"):
                    ...
                    update_flow(f"🔄 Iniciando procesamiento de la consulta sobre Dian Full...")
                    time.sleep(0.5)
                    ...
                    # Consultar directamente a Pinecone para Dian Full
                    try:
                        # Consultar directamente a Pinecone
                        print("DianFull.py: Consultando directamente a Pinecone (índice dianfull)")
                        documents = query_dianfull(query)
                        print(f"DianFull.py: Recuperados {len(documents)} documentos de Pinecone")
                        
                        # Verificar si se encontraron documentos
                        if not documents:
                            update_flow("❌ No se encontraron documentos relevantes en Pinecone")
                            response = "Lo siento, no encontré información relevante sobre tu consulta en la base de conocimiento de Dian Full. Por favor, intenta reformular tu pregunta o consulta otra base de conocimiento."
                            final_flow = '\n'.join(flow_steps)
...
with st.sidebar:
    st.header("Información sobre Dian Full")
    
    # Botón para limpiar la conversación
    if "dianfull_messages" in st.session_state:
